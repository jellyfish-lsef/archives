...
interface ResistorTextEditorProps {
  setMonaco: (editor: monaco.editor.IStandaloneCodeEditor) => void;
}

export class ResistorTextEditor extends Component<ResistorTextEditorProps, { editor: monaco.editor.IStandaloneCodeEditor}> {
    shouldComponentUpdate() { return false; }
    base?: HTMLElement;

    createEditorTab(path?: string, value?: string) {
        if (path && value == null) { value = fs.readFileSync(path, "utf8"); }
        let uri;
        if (path) {
            uri = monaco.Uri.file(path);
            if (monaco.editor.getModel(uri)) return this.state.editor.setModel(monaco.editor.getModel(uri));
    ...
    componentDidMount() {
        monacoPromise.then((monaco) => {
            const editor = monaco.editor.create(this.base as HTMLElement, {
                value: "Loading...",
