...
export class TabBar extends Component<{}, {editor: monaco.editor.IStandaloneCodeEditor}> {
    ...
    componentDidUpdate() {
        console.log("tabbar did render!");
        setTimeout(() => {
            // we'd think there'd be a better way to do this, but 🤷
            document.querySelector(".tabbar-tab.active")?.scrollIntoView({ behavior: "smooth", inline: "center" });
        });
    }
    constructor() {
