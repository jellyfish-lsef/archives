...
export async function killModel(model: monaco.editor.ITextModel) {
    ...
    const index = [...monaco.editor.getModels()].indexOf(model);
    model.dispose();
    setTimeout(() => {
        const models = [...monaco.editor.getModels()];
        let nModel = models[index - 1];
        if (!nModel) nModel = models[index];
        if (!nModel) nModel = models[0];
        const nextModel = inuse ? nModel : currentModel;
...
async function handleLinkClick(url: URL) {
    try {
        ...
        } else if (url.hostname == "dashboard.fluidity.developer.com" && url.searchParams.has("scriptID")) {
