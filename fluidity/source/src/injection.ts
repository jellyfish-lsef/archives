...
import { CALAMARI_API_LOCATION, DYLIB_UPDATE } from "./consts";
import { newComms, NewCommsConnection, NewCommsMessageType } from "./newcomms";
...
export async function ensureDependency() {
    ...
    if (!fs.existsSync("/usr/local/lib/libdependency_x86_64.dylib")) {
        console.log("Downloading dependency");
        while (true) {
            try {
                await downloadFile("https://cdn.fluidity.com/mac/libdependency_x86_64.1.dylib", "/usr/local/lib/libdependency_x86_64.dylib");
...
async function doInject(requestCustomDylib: boolean) {
    ...
    try {
        ...
        if (await ensureDependency() == false) return;
