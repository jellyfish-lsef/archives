...
export class EditorActions extends Component<{}, {injectionStatus: InjectionStatus, isSaving: boolean, lspStatus:any}> {
    render() {
        ...
        <div class="actionsPanel" id="editorActions" onwheel=${(event: WheelEvent) => (this.base?.firstChild as HTMLDivElement).scrollLeft += event.deltaX ? 0 : event.deltaY}>
            <${RunButtons} type="actionButton" getScript=${async() => (await editorPromise).getValue()}/>
            <button class="actionButton ${this.state.injectionStatus?.globalStatus ? "glowing" : ""}" onclick=${(evt:MouseEvent) => {
    console.log(ipcRenderer.invoke("inject", evt.shiftKey));
}} id="injectButton">
                <i class="material-symbols">memory</i>
                ${this.state.injectionStatus?.globalStatus || "Inject"}
            </button>

            <button class="actionButton" id="saveButton" isBusy=${this.state.isSaving} onclick=${this.save.bind(this)}>
                <i class="material-symbols">save</i>
                ${this.state.isSaving ? "Saving" : "Save"}
            </button>
<!--
            <button class="actionButton" id="saveButton">
